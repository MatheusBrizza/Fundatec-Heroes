...
class RemoteCharacterDataSource {
    ...
    suspend fun saveCharacterApi(
        userId: Int, name: String, description: String, image: String,
        universeType: String, characterType: String, age: Int, birthday: String?
    ): Int? {
        return withContext(Dispatchers.IO) {
            try {
                val userResponse = service.saveCharacter(
                    userId, CharacterRequest(
                        name, description, image, universeType, characterType, age, birthday
                    )
                )
